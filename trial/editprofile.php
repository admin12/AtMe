    		...
    		if (!empty($new_picture)) 
			{
		        ...
		        ($new_picture_width <= MM_MAXIMGWIDTH) && ($new_picture_height <= MM_MAXIMGHEIGHT)) 
				{
        			if ($_FILES['file']['error'] == 0) 
					{
						...
						$filename = 'users/' . $_SESSION['id'] . '/' . $username . $date . "." . end($ext);
...
?>
  ...
  <form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
      ...
      <input type="text" id="date" name="date" class="regbox" readonly="readonly" value="<?php if (!empty($date)) echo $date; ?>" />
	  </td></tr>
	  <tr height="50"><td>
	  	Username
	  </td><td>
	  <input type="text" id="username" class="regbox" name="username" readonly="readonly" value="<?php if (!empty($username)) echo $username; ?>" />
	  </td></tr>
	  <tr height="50"><td>
      		Name
      </td><td>
      <input type="text" id="name" name="name" class="regbox" value="<?php if (!empty($name)) echo $name; ?>" />
       </td></tr>
       <tr height="50"><td>
       		Location
       </td><td>
       <input type="text" id="location" name="location" class="regbox" value="<?php if (!empty($location)) echo $location; ?>" />
        </td></tr>
      <tr height="50"><td>
      	Date of Birth
      </td><td>
      ...
      <input type="text" id="email" class="regbox" name="email" value="<?php if (!empty($email)) echo $email; ?>" />
	  </td></tr>
      ...
      <input type="file" id="new_picture" name="new_picture" /><input type="checkbox" name="disp" id="disp" value="true" />Display Pic?<br />
      <?php 
    ...
    <tr><td><input type="submit" value="Save Profile" name="submit" /></td></tr>
    </table>
  </form>
<?php 
	...
	{
		echo 'This area is restricted to registered users.';
	} 
